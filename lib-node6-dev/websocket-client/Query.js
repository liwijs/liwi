...
let Query = class extends _AbstractQuery2.default {
  constructor(store, key) {
    ...
    _flowRuntime2.default.param('store', _storeType).assert(store), _flowRuntime2.default.param('key', _keyType).assert(key), super(store), _flowRuntime2.default.bindTypeParameters(this, _flowRuntime2.default.ref(_WebsocketStore2.default)), this.key = key;
  }
  ...
  fetch(callback) {
    ...
    return _flowRuntime2.default.param('callback', _callbackType).assert(callback), this.store.emit('fetch', this.key).then(callback).then(_arg => _returnType.assert(_arg));
  }
