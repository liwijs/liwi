...
let MongoStore = class extends _AbstractStore2.default {
  ...
  partialUpdateByKey(key, partialUpdate) {
    ...
    return _asyncToGenerator(function* () {
      ...
      partialUpdate = _partialUpdateType2.assert(_this._partialUpdate(partialUpdate));
      const collection = yield _this.collection;
      const commandResult = MongoUpdateCommandResultType.assert((yield collection.updateOne({ _id: key }, partialUpdate)));
      if (!commandResult.result.ok) {
        console.error(commandResult);
        throw new Error('Update failed');
      }
      return _returnType.assert(_this.findByKey(key));
    })();
  }
