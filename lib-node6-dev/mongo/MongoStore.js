...
let MongoStore = class extends _AbstractStore2.default {
  ...
  constructor(connection, collectionName) {
    ...
    if (_flowRuntime2.default.param('connection', _connectionType).assert(connection), _flowRuntime2.default.param('collectionName', _collectionNameType).assert(collectionName), super(connection), this.keyPath = '_id', _flowRuntime2.default.bindTypeParameters(this, _flowRuntime2.default.ref(_MongoConnection2.default)), !collectionName) throw new Error(`Invalid collectionName: "${collectionName}"`);

    this._collection = connection.getConnection().then(db => {
      let _dbType = _flowRuntime2.default.ref(_db2.default);

      return _flowRuntime2.default.param('db', _dbType).assert(db), this._collection = db.collection(collectionName);
  ...
  insertOne(object) {
    ...
    return _flowRuntime2.default.param('object', _objectType).assert(object), object._id || (object._id = new _mongodb.ObjectID().toString()), object.created || (object.created = new Date()), this.collection.then(collection => collection.insertOne(object)).then(({ result, connection, ops }) => {
  ...
  replaceOne(object) {
    ...
    return _flowRuntime2.default.param('object', _objectType2).assert(object), object.updated || (object.updated = new Date()), this.collection.then(collection => collection.updateOne({ _id: object._id }, object)).then(() => object).then(_arg4 => _returnType4.assert(_arg4));
  }
  ...
  upsertOne(object) {
    ...
    return _flowRuntime2.default.param('object', _objectType3).assert(object), object.updated || (object.updated = new Date()), this.collection.then(collection => collection.updateOne({ _id: object._id }, { $set: object }, { upsert: true })).then(() => object).then(_arg5 => _returnType5.assert(_arg5));
  }
  ...
  replaceSeveral(objects) {
    ...
    return _flowRuntime2.default.param('objects', _objectsType).assert(objects), Promise.all(objects.map(object => this.updateOne(object))).then(_arg6 => _returnType6.assert(_arg6));
  }
  ...
  _partialUpdate(partialUpdate) {
    ...
    return _flowRuntime2.default.param('partialUpdate', _partialUpdateType).assert(partialUpdate), Object.keys(partialUpdate).some(key => key[0] === '$') ? partialUpdate : { $set: partialUpdate };
  }
  ...
  partialUpdateByKey(key, partialUpdate) {
    ...
    return _flowRuntime2.default.param('key', _keyType).assert(key), _flowRuntime2.default.param('partialUpdate', _partialUpdateType2).assert(partialUpdate), partialUpdate = _partialUpdateType2.assert(this._partialUpdate(partialUpdate)), this.collection.then(collection => collection.updateOne({ _id: key }, partialUpdate)).then(_arg7 => _returnType7.assert(_arg7));
  }
  ...
  partialUpdateOne(object, partialUpdate) {
    ...
    return _flowRuntime2.default.param('object', _objectType4).assert(object), _flowRuntime2.default.param('partialUpdate', _partialUpdateType3).assert(partialUpdate), partialUpdate = _partialUpdateType3.assert(this._partialUpdate(partialUpdate)), this.partialUpdateByKey(object._id, partialUpdate).then(() => this.findByKey(object._id)).then(_arg8 => _returnType8.assert(_arg8));
  }
  ...
  partialUpdateMany(criteria, partialUpdate) {
    ...
    return _flowRuntime2.default.param('partialUpdate', _partialUpdateType4).assert(partialUpdate), partialUpdate = _partialUpdateType4.assert(this._partialUpdate(partialUpdate)), this.collection.then(collection => collection.updateMany(criteria, partialUpdate)).then(() => null).then(_arg9 => _returnType9.assert(_arg9)); // TODO return updated object
  }
  ...
  deleteByKey(key) {
    ...
    return _flowRuntime2.default.param('key', _keyType2).assert(key), this.collection.then(collection => collection.removeOne({ _id: key })).then(() => null).then(_arg10 => _returnType10.assert(_arg10));
  }
  ...
  cursor(criteria, sort) {
    ...
    return _flowRuntime2.default.param('criteria', _criteriaType).assert(criteria), _flowRuntime2.default.param('sort', _sortType).assert(sort), this.collection.then(collection => collection.find(criteria)).then(sort && (cursor => cursor.sort(sort))).then(cursor => new _MongoCursor2.default(this, cursor)).then(_arg11 => _returnType11.assert(_arg11));
  }
  ...
  findByKey(key) {
    ...
    return _flowRuntime2.default.param('key', _keyType3).assert(key), this.findOne({ _id: key }).then(_arg12 => _returnType12.assert(_arg12));
  }
  ...
  findOne(criteria, sort) {
    ...
    return _flowRuntime2.default.param('criteria', _criteriaType2).assert(criteria), _flowRuntime2.default.param('sort', _sortType2).assert(sort), this.collection.then(collection => collection.find(criteria)).then(sort && (cursor => cursor.sort(sort))).then(cursor => cursor.limit(1).next()).then(_arg13 => _returnType13.assert(_arg13));
  }
};
