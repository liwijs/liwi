...
var _excluded = ["url"];
var logger = new nightingaleLogger.Logger('liwi:resources-websocket-client');
...
function createResourcesWebsocketClient(_ref2) {
  var url = _ref2.url,
      options = _objectWithoutPropertiesLoose__default(_ref2, _excluded);
  ...
  var wsClient = createSimpleWebsocketClient(_extends__default({}, options, {
    ...
    onMessage: function onMessage(event) {
      ...
      var _decode = extendedJson.decode(event.data),
          type = _decode[0],
          id = _decode[1],
          error = _decode[2],
          result = _decode[3];
  ...
  var sendMessage = function sendMessage(type, id, payload) {
    wsClient.sendMessage(extendedJson.encode([type, id, payload]));
  };
