    ...
    socket.on('disconnect', () => {
      ...
      try {
        if (json) {
          ...
          if (callback = _callbackType.assert(args), args = _argsType.assert(decode(json)), !Array.isArray(args) && (logger.debug('args', { args }), callback)) throw new Error('Invalid args');
        }
