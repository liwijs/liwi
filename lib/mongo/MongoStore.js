...
let MongoStore = class MongoStore extends _AbstractStore2.default {
    ...
    */
    ...
    constructor(connection, collectionName) {
        ...
        this._collection = connection.getConnection().then(connection => this._collection = connection.collection(collectionName));
    }
    ...
    */insertOne(object) {
        ...
        return Promise.resolve(this._collection).then(collection => collection.insertOne(object)).then(_ref => {
    ...
    */updateOne(object) {
        ...
        return Promise.resolve(this._collection).then(collection => collection.updateOne({ _id: object._id }, object)).then(() => object);
    }
    ...
    */partialUpdateByKey(key, partialUpdate) {
        ...
        return Promise.resolve(this._collection).then(collection => collection.updateOne({ _id: key }, partialUpdate));
    }
    ...
    */partialUpdateMany(criteria, partialUpdate) {
        partialUpdate = this._partialUpdate(partialUpdate);
        return Promise.resolve(this._collection).then(collection => collection.updateMany(criteria, partialUpdate)).then(res => null); // TODO return updated object
    }
     ...
     * @returns {Promise}
    */deleteByKey(key) {
        return Promise.resolve(this._collection).then(collection => collection.removeOne({ _id: key })).then(() => null);
    }
    ...
    */cursor(criteria) {
        return Promise.resolve(this._collection).then(collection => collection.find()).then(cursor => new _MongoCursor2.default(this, cursor, criteria));
    }
    ...
    */findByKey(key) {
     ...
     * @param {Object} criteria
    */findOne(criteria) {
        return Promise.resolve(this._collection).then(collection => collection.find(criteria).limit(1).next());
    }
};
